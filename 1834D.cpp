...
void solution()
{
    ...
    ll i1, i2;
    rep(i, 0, n) {
        if(arr[i].ff > mx) mx = arr[i].ff, i1 = i;
        if(arr[i].ss < mn) mn = arr[i].ss, i2 = i;
    }
    ll i3;
    mn = INT_MAX;
    rep(i, 0, n) {
        if(arr[i].ss - arr[i].ff < mn) mn = arr[i].ss - arr[i].ff, i3 = i;
    }
    ...
    rep(i, 0, n) {
        for(auto idx : p) {
            ll l = max(arr[i].ff, arr[idx].ff), r = min(arr[i].ss, arr[idx].ss);
            if(l > r) ans = max(ans, 2 * (arr[i].ss - arr[i].ff + 1));
            else ans = max(ans, 2 * (arr[i].ss - arr[i].ff - (r - l)));
        }
    }
