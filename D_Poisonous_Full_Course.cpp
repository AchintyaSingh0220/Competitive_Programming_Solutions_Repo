...
ll func(int i, int f, vi &poi, vi &taste) {
    if(i == poi.size()) return 0;
    if(dp[i][f] != -1) return dp[i][f];

    ll ret = INT_MIN;
    ret = max(ret, func(i + 1, f, poi, taste));

    if(poi[i] == 0) ret = max(ret, func(i + 1, 0, poi, taste) + taste[i]);
    else {
        if(f == 0) ret = max(ret, func(i + 1, 1, poi, taste) + taste[i]);
    }
